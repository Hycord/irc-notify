...
Filter values support template variable resolution:

```typescript
{
  field: "message.content",
  operator: "contains",
  value: "{{metadata.clientNickname}}"  // Resolved at runtime
}
```
...
**Example:**
```typescript
// context.metadata.clientNickname = "mybot"
{
  field: "message.content",
  operator: "matches",
  pattern: "@{{metadata.clientNickname}}\\b"
}
// Matches: "@mybot" in message content
```
