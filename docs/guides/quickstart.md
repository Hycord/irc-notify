...
Create `config/clients/textual.ts`:
...
Create `config/servers/libera.ts`:
...
Create `config/sinks/console.ts`:
...
Create `config/events/phrase-alert.ts`:
...
Loaded configuration from config/config.ts
  - 1 clients
  - 1 servers
  - 1 events
  - 1 sinks
...
Create `config/events/direct-message.ts`:
...
- [TypeScript Configuration](./typescript-config.md) - Config system details
- [Strict Types](./strict-types.md) - Type safety and autocomplete
