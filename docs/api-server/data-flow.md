...
**Client Association Logic**: A server is associated with a client based on **actual discovery at runtime**:
- The client's discovery mechanism (filesystem, JSON, or static) finds this specific server
- The server's hostname matches a hostname in the client's discovered servers list
- Matching is case-insensitive by hostname

**Important**: This reflects actual runtime behavior, not just configuration potential. For example:
- A Textual client (filesystem discovery) that has log directories for 3 servers will show associations with only those 3 servers
- A TheLounge client (JSON discovery) configured with 1 server will show an association with only that 1 server
- Multiple clients can monitor the same server if both discover it (e.g., both Textual and TheLounge monitoring Libera.Chat)
  ...
  "servers": [
    {
      ...
      "port": 6697,
      "enabled": true,
      "clientIds": ["textual", "thelounge"],
      "clientNames": ["Textual IRC Client", "The Lounge Web IRC"],
      "usersCount": 5,
      "users": [
        {
          "nickname": "alice",
...
Each server in the `servers` array includes `clientIds` and `clientNames` arrays showing which clients actually discovered that server at initialization:

- **Discovery-based matching**: The endpoint queries each initialized client adapter's `getDiscoveredServers()` method to get the actual list of servers found
- **Hostname matching**: Servers are matched by comparing the configured `hostname` field (case-insensitive) against discovered server hostnames
- **Runtime accuracy**: Associations reflect what's actually on disk/configured, not theoretical configuration

**Example scenarios:**
- Textual client with log directories for Libera, Orpheus, and RED → shows 3 associations
- TheLounge client with only Libera configured in config.js → shows 1 association (Libera only)
- Both clients monitoring Libera → Libera server shows 2 client associations

This information helps understand:
- Which clients will actually emit messages for each server
- How log files are currently being monitored
- Whether servers are redundantly monitored by multiple clients
- Configuration mismatches (e.g., server config exists but no client discovered it)
