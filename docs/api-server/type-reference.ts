 ...
 * Returns the root IRCNotifyConfig object
 */
export type RootConfigGetResponse = IRCNotifyConfig;

/**
 * Response from PUT /api/config
 */
export interface RootConfigPutResponse {
  updated: boolean;
 ...
 * Comprehensive analysis of message flow through the system
 */
...
/**
 * Client adapter configuration with discovery and parsing rules
 */
...
/**
 * Server configuration with user metadata
 */
...
/**
 * Event configuration with filters and sink routing
 */
...
/**
 * Sink configuration with notification delivery details
 */
...
/**
 * Complete routing path from client to sink with enabled/disabled tracking
 * The 'enabled' field is true only if ALL components in the path are enabled
 */
export interface DataFlowRoutingPath {
  ...
  sinkIds: string[];
  sinkStatuses: Array<{ id: string; enabled: boolean }>;
  enabled: boolean;
}
